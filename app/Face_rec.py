...
log.info("[INFO] Importing pretrained model..")
...
face_encoder = dlib.face_recognition_model_v1(
    "./Predictor/dlib_face_recognition_resnet_model_v1.dat"
)
...
def easy_face_reco(frame, known_face_encodings, known_face_names):
    ...
    if (
        face_encodings_list is not None
...
def images_process(path=Path("images")):
    log.info("Loading images to recognize...")
    ...
    for file_ in files:
        ...
        face_encodings, _, _ = encode_face(image)
        if face_encodings:
            face_encoded = face_encodings[0]
            known_face_encodings.append(face_encoded)
            log.info(f"Face encoding found for {file_}")
        else:
